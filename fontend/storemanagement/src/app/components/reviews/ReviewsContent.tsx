...
import { ReviewTab, STAR_LABELS, Product, Category } from "@/src/lib/data";
    ...
    >
      ...
      >
        ...
        >
          ...
          <strong style={{ color: "#1e293b" }}>{state.label}</strong>? This
        ...
        >
          <BtnTeal
            ...
            style={{ background: "#ef4444", padding: ".6rem 1.5rem" }}
          >
            Yes, Delete
          </BtnTeal>
          <BtnOutline
            onClick={onClose}
            style={{ padding: ".6rem 1.5rem" } as React.CSSProperties}
          >
            Cancel
          </BtnOutline>
        </div>
  ...
  return (
    ...
    >
      ...
      >
        <span
          className="material-symbols-outlined"
          style={{ fontSize: "18px", color: "#166534" }}
        >
          check
        </span>
      </div>
      <div style={{ flex: 1 }}>
        <p style={{ fontWeight: 600, fontSize: ".875rem", color: "#166534" }}>
          {message}
        </p>
      </div>
      <button
...
export default function ReviewsContent() {
  ...
  const submitReview = async () => {
    ...
    try {
      const payloadComment = wrapComment(comment, showName);
      if (editingReviewId) {
  ...
  const startEdit = (r: ApiReview) => {
    ...
    const { text, isAnonymous } = unwrapComment(r.comment);
    setEditingReviewId(r.reviewId);
    setPicked(p);
    setStar(r.rating);
    setComment(text);
    setShowName(!isAnonymous);
    setTab("write");
  };
  ...
  const mineTotalPages = Math.max(
    1,
    Math.ceil(myReviews.length / MINE_PAGE_SIZE),
  );
  ...
  const mineGoTo = (p: number) =>
    setMinePage(Math.max(1, Math.min(mineTotalPages, p)));
  let mLo = Math.max(1, mineSafePage - 2);
  let mHi = Math.min(mineTotalPages, mineSafePage + 2);
  if (mHi - mLo < 4) {
    if (mLo === 1) mHi = Math.min(mineTotalPages, mLo + 4);
    else mLo = Math.max(1, mHi - 4);
  }
  ...
  const tabs: { id: ReviewTab; icon: string; label: string }[] = [
    {
      id: "pending",
      icon: "schedule",
      label: `Pending (${pendingPids.length})`,
    },
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "2rem 1.5rem" }}>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        <h1 className="serif" style={{ fontSize: "2rem", fontWeight: 700 }}>
          Product Reviews
        </h1>
        ...
        <div>
          ...
          {!hasDeliveredOrders && (
            ...
            >
              ...
              >
                You need to purchase and receive a product before you can write
                a review.
              </p>
              <BtnTeal onClick={() => (window.location.href = "/shop")}>
                Start Shopping
              </BtnTeal>
            </div>
          )}
          ...
          {hasDeliveredOrders && pendingPids.length === 0 && (
            ...
            >
              <span
                className="material-symbols-outlined"
                style={{ fontSize: "3.5rem", color: "#16a34a" }}
              >
                check_circle
              </span>
              <p
          ...
          {pendingPids.length > 0 && (
            <div>
              ...
              >
                These products from your delivered orders are waiting for your
                review:
              </p>
              <div
                style={{
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: "flex", gap: 2, marginTop: 4 }}>
                          {Array.from({ length: 5 }, (_, i) => (
                            <span
                              key={i}
                              style={{ color: "#e2e8f0", fontSize: ".9rem" }}
                            >
                              ★
                            </span>
                          ))}
                      ...
                      <BtnTeal
                        ...
                        style={{ fontSize: ".8rem", padding: "0.5rem 1rem" }}
                      >
                        Write Review
                      </BtnTeal>
                    </div>
        ...
        <div>
          ...
          {!hasDeliveredOrders && !picked && (
            ...
            >
              ...
              >
                You can only review products that you have purchased and
                received.
              </p>
              <BtnTeal onClick={() => (window.location.href = "/shop")}>
                Go to Shop
          ...
          {picked && (
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: ".75rem", color: "#64748b" }}>
                    {categoryName(picked.categoryId)}
                  </p>
                </div>
                <button
                  onClick={resetForm}
              ...
              <div style={{ marginBottom: "1.25rem" }}>
                  ...
                  {[1, 2, 3, 4, 5].map((n) => (
                    <span
                      ...
                      style={{
                        fontSize: "2rem",
                        cursor: "pointer",
                        color: n <= displayStar ? "#f59e0b" : "#e2e8f0",
                        transition: ".15s",
                      }}
                    >
                      ★
                ...
                <p
                  style={{ fontSize: ".75rem", color: "#94a3b8", marginTop: 4 }}
                >
              ...
              <div style={{ display: "flex", gap: "0.75rem" }}>
                <BtnTeal
                  ...
                  style={{
                    flex: 1,
                    padding: "0.75rem",
                    opacity: !star || !comment.trim() ? 0.6 : 1,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    gap: 6,
          ...
          {myReviews.length === 0 ? (
            ...
            >
              {pagedReviews.map((r) => {
                const p = products.find((x) => x.id === r.productId);
                if (!p) return null;
                const { text: reviewText, isAnonymous } = unwrapComment(r.comment);
                return (
                  <div
                    key={r.reviewId}
                    style={{
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p style={{ fontSize: ".75rem", color: "#94a3b8" }}>
              ...
              {mineTotalPages > 1 && (
                ...
                >
                  <button
                    onClick={() => mineGoTo(mineSafePage - 1)}
                    disabled={mineSafePage === 1}
                    style={{
                      width: 34,
                      height: 34,
                      borderRadius: "50%",
                      border: "1.5px solid",
                      borderColor:
                        mineSafePage === 1 ? "#e2e8f0" : "var(--teal)",
                      background: "#fff",
                      color: mineSafePage === 1 ? "#cbd5e1" : "var(--teal)",
                      cursor: mineSafePage === 1 ? "default" : "pointer",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                    }}
                  ...
                  {minePageNums.map((n) => (
                    <button
                      ...
                      style={{
                        ...
                        border: "1.5px solid",
                        borderColor:
                          n === mineSafePage ? "var(--teal)" : "#e2e8f0",
                        background: n === mineSafePage ? "var(--teal)" : "#fff",
                        color: n === mineSafePage ? "#fff" : "#374151",
                        fontWeight: n === mineSafePage ? 700 : 400,
                        cursor: "pointer",
                        fontSize: ".875rem",
                      }}
                    >
                  ...
                  <button
                    onClick={() => mineGoTo(mineSafePage + 1)}
                    disabled={mineSafePage === mineTotalPages}
                    style={{
                      width: 34,
                      height: 34,
                      borderRadius: "50%",
                      ...
                      color:
                        mineSafePage === mineTotalPages
                          ...
                          : "var(--teal)",
                      cursor:
                        mineSafePage === mineTotalPages ? "default" : "pointer",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                    }}
