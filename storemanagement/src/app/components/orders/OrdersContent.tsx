            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              {([['Order Placed', true], ['Payment Confirmed', detailOrder.status !== 'cancelled'], ['Packed & Ready', ['shipping', 'delivered'].includes(detailOrder.status)], ['Out for Delivery', detailOrder.status === 'delivered'], ['Delivered', detailOrder.status === 'delivered']] as [string, boolean][]).map(([l, done]) => (
                <div key={l} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: done ? 'var(--teal)' : '#f1f5f9', color: done ? '#fff' : '#94a3b8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.8rem', fontWeight: 700, flexShrink: 0 }}>
                    {done ? '✓' : '○'}
                  </div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                {detailOrder.status === 'delivered' && (
                  <BtnTeal onClick={() => { setDetailOrder(null); router.push('/reviews'); }} style={{ flex: 1, padding: '0.6rem' }}>
                    ⭐ Write Review
                  </BtnTeal>
                )}
                {detailOrder.status === 'shipping' && (
                  <BtnTeal style={{ flex: 1, padding: '0.6rem' }}>🗺 Track Shipment</BtnTeal>
                )}
              ...
              <div key={o.id} style={{ background: '#fff', borderRadius: '1.25rem', boxShadow: '0 2px 10px rgba(0,0,0,.05)', overflow: 'hidden', transition: '.2s' }}>
                <div style={{ padding: '1.25rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    ...
                    {o.status === 'delivered' && (
                      <BtnOutline onClick={() => router.push('/reviews')} style={{ fontSize: '.8rem', padding: '0.4rem .9rem' } as React.CSSProperties}>⭐ Write Review</BtnOutline>
                    )}
                    {o.status === 'shipping' && (
                      <BtnOutline style={{ fontSize: '.8rem', padding: '0.4rem .9rem' } as React.CSSProperties}>🗺 Track Order</BtnOutline>
                    )}
                    {o.status === 'processing' && (
